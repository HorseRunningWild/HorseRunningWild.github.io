...
- **Asset pipeline**: CSS/JS in `assets/` (`css/homepage.css`, `css/resources.css`, `js/homepage-gsap.js`, `js/neuron-bg.js`, `js/hero-name-toggle.js`) processed through Hugo Pipes (`minify | fingerprint`); images via `resources.Get` (`avatar.jpg`, `avatar-homepage-hero/bg.png` + `fg.png`).
