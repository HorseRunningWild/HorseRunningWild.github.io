...
- **Publication cards**: homepage Publications section ("Journal & Conference") uses native `<details class="pub-card">` accordions — summary carries badge/title/authors/venue/desc, the expanded body adds an abstract paragraph, review scores (NSR), and a click-to-cycle figure gallery (`js/pub-cards.js`; figures live in `assets/pub-*/`, captions included). The accepted NSR paper PDF is served from `static/papers/`.
